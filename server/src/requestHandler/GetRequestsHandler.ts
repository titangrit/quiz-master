...
import { GetEndpoint, QuizLifeCycleStatusCode, QuizMasterSchema } from "./constants";
import { IHandleDatabase, GetParam } from "./IHandleDatabase";
import { ViewQuizData, GetAllQuizzes, QuizBasicInfo, QuizTeamsDetail, RoundQuestionsDetail } from "./types";
...
export class GetRequestsHandler {
    ...
    async handleGetRequest(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (req.params[0] === GetEndpoint.round_types) {
                ...
                const roundTypes: GetParam.RoundType[] = await this.getRoundTypes(roundTypeID);
    ...
    private async getRoundTypes(roundTypeId?: string): Promise<GetParam.RoundType[]> {
        try {
            if (roundTypeId) {
                const roundTypes: GetParam.RoundType[] = [];
                const result: GetParam.RoundType = await this.dbHandler.getRoundTypeByID(roundTypeId);
                roundTypes.push(result);
                return roundTypes;
            } else {
                const roundTypes: GetParam.RoundType[] = await this.dbHandler.getRoundTypes();
                return roundTypes;
            }
        } catch (error) {
