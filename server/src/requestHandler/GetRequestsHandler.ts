...
import { IHandleDatabase, GetParam } from "./IHandleDatabase";
import {
  ViewQuizData,
  GetAllQuizzes,
  QuizBasicInfo,
...
export class GetRequestsHandler {
  private dbHandler: IHandleDatabase;
  ...
  async handleGetRequest(req: Request, res: Response): Promise<void> {
    try {
      const db = await MySqlDbHandler.getInstance();

      if (req.params[0] === GetEndpoint.round_types) {
        const roundTypeID = req.query.roundTypeID as string;
        const roundTypes: GetParam.RoundType[] = await this.getRoundTypes(
          roundTypeID
        );
        res.json({ RoundTypes: roundTypes });
      } else if (req.params[0] === GetEndpoint.quiz_round_details) {
      ...
      } else if (req.params[0] === GetEndpoint.view_quiz_data) {
        const _quizID = req.query.quizID as string;
        const quizID = parseInt(_quizID);
        ...
        try {
          ...
          for (const teamUUID of teamUUIDs) {
            ...
            const team: GetParam.Team = await db.getTeamByUUID(
              teamUUID as string
            );
            teams.push(team);
          }
          ...
          for (const round of _rounds) {
            // Round type detail
            const _roundType: GetParam.RoundType = await db.getRoundTypeByID(
              round.RoundTypeID
            );

            // Questions Detail
            let _questions: GetParam.Question[] =
              await db.getQuestionsByRoundUUID(round.UUID);
            ...
            questions = questions.slice(
              0,
              _roundType.NumQuestionsEachTeam * quiz.NumberOfTeams
            );
      ...
      } else if (req.params[0] === GetEndpoint.all_quizzes) {
      ...
      } else if (req.params[0] === GetEndpoint.basic_info) {
        try {
          const _quizID = req.query.quizID as string;
          const quizID = parseInt(_quizID);
      ...
      } else if (req.params[0] === GetEndpoint.team_info) {
        try {
          const _quizID = req.query.quizID as string;
          const quizID = parseInt(_quizID);
      ...
      } else if (req.params[0] === GetEndpoint.rounds) {
        try {
          const _quizID = req.query.quizID as string;
          const quizID = parseInt(_quizID);
          ...
          let currentRounds: GetParam.Round[] = await db.getRoundsByQuizID(
            quizID
          );
      ...
      } else if (req.params[0] === GetEndpoint.questions) {
        try {
          ...
          const roundType: GetParam.RoundType = await db.getRoundTypeByID(
            round.RoundTypeID
          );
          const quiz: GetParam.Quiz = await db.getQuizByID(round.QuizID);
          const totalNumQuestions =
            roundType.NumQuestionsEachTeam * quiz.NumberOfTeams;

          const currentQuestions: GetParam.Question[] =
            await db.getQuestionsByRoundUUID(roundUUID);
      ...
      } else if (req.params[0] === GetEndpoint.about_quiz) {
        const _quizID = req.query.quizID as string;
        const quizID = parseInt(_quizID);
  ...
  private async getRoundTypes(
    roundTypeId?: string
  ): Promise<GetParam.RoundType[]> {
    try {
      if (roundTypeId) {
        const roundTypes: GetParam.RoundType[] = [];
        const result: GetParam.RoundType =
          await this.dbHandler.getRoundTypeByID(roundTypeId);
        roundTypes.push(result);
        return roundTypes;
      } else {
        const roundTypes: GetParam.RoundType[] =
          await this.dbHandler.getRoundTypes();
        return roundTypes;
      }
    } catch (error) {
      logger.error("GetRequestsHandler.getRoundTypes -> ", error);
      throw error;
    }
  }
}
