    ...
    if (req.params[0] === PostEndpoint.QuizBasicInfo) {
      try {
        if (req.body.QuizEventID) {
          ...
          // New quiz
          const param: CreateParam.QuizInstance = {
      ...
      try {
        ...
        for (let _t = 0; _t < teams.length; _t++) {
          ...
          for (let _m = 0; _m < 4; _m++) {
            if (
              !teams[_t].Members[_m].Surname &&
              !teams[_t].Members[_m].Name &&
              !teams[_t].Members[_m].Lastname
            ) {
              continue;
            }
            const param: CreateParam.MemberInstance = {
          ...
          const param: CreateParam.TeamInstance =
            {} as CreateParam.TeamInstance;
      ...
      try {
        ...
        for (let _r = 0; _r < rounds.length; _r++) {
          if (rounds[_r].RoundTypeName) {
            ...
            const param: CreateParam.RoundTypeInstance = {
          ...
          if (rounds[_r].UUID) {
            ...
            const param: UpdateParam.RoundInstance = {
              RoundTypeID: rounds[_r].RoundTypeID,
            };
            ...
            // It is new round
            const param: CreateParam.RoundInstance = {
              QuizID: quizID,
              RoundTypeID: rounds[_r].RoundTypeID,
        ...
        for (let _q = 0; _q < questions.length; _q++) {
          if (questions[_q].UUID) {
            ...
            if (questions[_q].MediaUpdated) {
              const file: Express.Multer.File = media.find(
                (x: Express.Multer.File) =>
                  x.originalname == questions[_q].SequenceNumber
              );
            ...
            const param: CreateParam.QuestionInstance = {
              RoundUUID: roundUUID,
